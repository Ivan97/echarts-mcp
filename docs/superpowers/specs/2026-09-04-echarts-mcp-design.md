...
| option 不合法 | **不能靠异常**，见 §16.9。改为检测空图并给出结构化提示 |
...
### 16.9 「捕获 setOption 异常」这条路走不通（实测推翻）

§10 原设计为「`setOption` 抛错时捕获，明确指出哪个 option 字段不合法」。
**实测推翻了这个前提**：ECharts 对非法 option 几乎从不抛异常，而是静默渲染出一张空图。

实测样本（echarts 6.1.0，SVG SSR，600x400）：

| 情形 | 是否抛错 | SVG 字节 | 可绘制元素数 |
|---|---|---|---|
| 正常柱状图 | 否 | 2473 | 8 |
| 单点折线 | 否 | 2875 | 10 |
| 仪表盘 | 否 | 11175 | 74 |
| `series` 传成字符串 | **否** | 233 | 0 |
| option 完全为空 | **否** | 233 | 0 |
| `series.type` 拼错 | **否**（仅 console 警告） | 604 | 1 |
| `series.data` 为空数组 | **否** | 441 | 1 |

改为**主动检测空图**：统计产物中 `path` / `circle` / `polyline` / `polygon` 的数量，
不超过 2 个即判定为空图。正常图最少 8 个、坏图最多 1 个，安全边界充足。

检测到空图时，在工具响应的 `notes` 中给出具体的排查方向
（series 是否为数组、type 是否拼错、data 是否为空、类目轴与数据长度是否匹配），
