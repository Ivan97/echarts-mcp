...
| `line` 折线图 | 37 | 37 | 全部通过 |
| `pie` 饼图 | 15 | 15 | 全部通过 |
| `scatter` 散点图 | 21 | 21 | 全部通过 |
| `radar` 雷达图 | 5 | 5 | 全部通过 |
| `heatmap` 热力图 | 3 | 3 | 全部通过 |
| `boxplot` 箱线图 | 3 | 3 | 全部通过 |
| `candlestick` K 线图 | 8 | 8 | 全部通过 |
| `funnel` 漏斗图 | 4 | 4 | 全部通过 |
| `sankey` 桑基图 | 7 | 7 | 全部通过 |
| `treemap` 矩形树图 | 7 | 7 | 全部通过 |
| `sunburst` 旭日图 | 7 | 7 | 全部通过 |
| `graph` 关系图 | 11 | 11 | 全部通过 |
| `tree` 树图 | 7 | 7 | 全部通过 |
| `parallel` 平行坐标 | 4 | 4 | 全部通过 |
| `calendar` 日历图 | 8 | 8 | 全部通过 |
| `matrix` 矩阵图 | 10 | 10 | 全部通过 |
